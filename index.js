...
FileTransport.prototype.send = function(mail, cb) {
  ...
  if (Array.isArray(data.to)) {
    for (let i = 0; i < data.to.length; i++) {
      err = this.saveEmailToFile(data.to[i], data.subject, data.html || data.text, data.attachments);
      if (err) break;
    }
  } else {
    err = this.saveEmailToFile(data.to, data.subject, data.html || data.text, data.attachments);
  }
